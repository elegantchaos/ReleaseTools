...
struct NotarizeCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        let result = try xcrun.run(arguments: ["altool", "--notarize-app", "--primary-bundle-id", parsed.archive.identifier, "--username", parsed.user, "--password", "@keychain:AC_PASSWORD", "--team-id", parsed.archive.team, "--file", parsed.exportedZipURL.path, "--output-format", "xml"])
        if result.status != 0 {
